...
def determine_thermal_upgrades(
    ...
    while (len(overloaded_line_list) > 0 or len(overloaded_xfmr_list) > 0) and (
        ...
        logger.info(
            f"Iteration_{iteration_counter}: Number of devices with violations after this iteration: Transformers:{len(overloaded_xfmr_list)}, Lines: {len(overloaded_line_list)}")
        iteration_counter += 1
    ...
    line_upgrades_df = line_upgrades_df.drop_duplicates(
        subset=["upgrade_type", "action", "final_equipment_name"], keep="last")
    xfmr_upgrades_df = xfmr_upgrades_df.drop_duplicates(
        subset=["upgrade_type", "action", "final_equipment_name"], keep="last")
    # validate upgrades output models
    m = AllUpgradesTechnicalResultModel(line=line_upgrades_df.to_dict(
        'records'), transformer=xfmr_upgrades_df.to_dict('records'))
